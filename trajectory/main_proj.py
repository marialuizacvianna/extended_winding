...
for t in np.arange(tdomain.lb(),tdomain.ub(),dt):
  ...
  if i%1 == 0:
    ...
    y = (x_truth(t)[1])
    theta = (x_truth(t)[2])
    # F = Function("x", "y", "(x - %f)^2 + (y - %f)^2 - %f^2"%(x,y,L))
    ...
    # seps.append(SepFwdBwd(F, CmpOp.LEQ))
    X += [Interval(x)]
    Y += [Interval(y)] 
  i += 1
...
# SIVIA(world, sep, 3)
beginDrawing()
# x = (x_truth(tdomain.lb())[0])
# y = (x_truth(tdomain.lb())[1])
# F = Function("x", "y", "(x - %f)^2 + (y - %f)^2 - %f^2"%(x,y,L))
# sep = SepFwdBwd(F, Interval(-oo,0))
# print("x = ", x)
# print("y = ", y)
...
a = SIVIA(world, sep,1., return_result = True)
...
print(len(a[SetValue.UNKNOWN]))
max_area = 0
for b in a[SetValue.UNKNOWN]:
  if(b[0].diam()*b[1].diam() > max_area):
    max_area = b[0].diam()*b[1].diam()
